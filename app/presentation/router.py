...
from app.presentation.schema import UserRegisterRequest, UserDeletionRequest, PostCreationRequest, PostCreationResponse, \
    UserPosts, NewPostRequest
...
async def create_post(request: PostCreationRequest):
    event = events.PostCreated(title=request.title, content=request.content,
                               user_id=request.user_id)  # TODO(humphrey): add user password
...
async def update_post(post_id: str, new_post: NewPostRequest):
    [updated_post] = await handle(
        events.PostUpdated(id=post_id, new_post=Post(id=post_id, title=new_post.title, content=new_post.content,
...
async def delete_post(post_id: str, request: PostDeleteRequest):
    await handle(events.PostDeleted(id=post_id))  # TODO(humphrey): add user password and id
